...
<tr id="trow_{{ $row->id }}">
    ...
    <td><strong> <a href="{{ url('admin/blogs/'.$row->id)}}">{{  $row->title }} </a></strong></td>
    <td><strong>{{ date('d M,Y',strtotime($row->created_at)) }}</strong></td>
     <td>
        <a class="btn btn-default btn-rounded btn-sm" href="{{ url('admin/blogs/'.$row->id.'/edit') }}" ><span class="fa fa-pencil"></span></a>
        <!--       
