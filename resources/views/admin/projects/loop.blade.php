...
<tr id="trow_{{ $row->id }}">
    ...
    <td><strong> <a href="{{ url('admin/projects/'.$row->id)}}">{{  $row->name }}</a></strong></td>
    <td><strong>{{ date('d M,Y',strtotime($row->created_at)) }}</strong></td>
     <td>
        <a class="btn btn-default btn-rounded btn-sm" href="{{ url('admin/projects/'.$row->id.'/edit') }}" ><span class="fa fa-pencil"></span></a>
        <!--       
       <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row('trow_{{ $row->id }}');"><span class="fa fa-times"></span></button>-->
        {!! Form::open(['action'=>['ProjectController@destroy',$row->id],'method'=>'delete' ,'style'=>'display: inline']) !!}
