...
class MessageController extends Controller
{
    ...
    public function index()
    {
        if (is_null($this->request->value)) {
            ...
            $this->request->flash();
            $services = $this->model->where('name', 'like', "%{$this->request->value}%")
                ->paginate(25);
        }
    ...
    {
        $users = Contactus::select('id', 'name', 'email', 'created_at')->get();
        Excel::create('msgs', function($excel) use($users) {
    ...
    public function show($id) {
        ...
        return View($this->view . 'show', compact('messages'));
    }
