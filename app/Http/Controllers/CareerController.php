...
//=========== add by my ==========//
use App\Http\Requests\CareerRequest;  //============ request for this controller ==========//
use App\Slider;   //========= admin model =============//
use App;

class CareerController extends Controller {
     //=======  request and model and view file =============//
    public function __construct(CareerRequest $request, Career $model)
    {
        $this->request = $request;
        $this->model = $model;
        $this->view = 'admin/career/';
    }
    ...
    public function index() {
        if (is_null($this->request->value)) {
            ...
            $slider = Career::where('title', 'like', "%{$this->request->value}%")
                    ->paginate(25);
        }
        $slider->setPath('career');
    ...
    public function store() {
        ...
        if ($insert) {
            if ($this->request->ajax())
                return response()->json(array('status' => 'true', 'message' => "Add Process done Sucessfully"));
